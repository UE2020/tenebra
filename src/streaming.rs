            ...
            if connection_state == RTCIceConnectionState::Failed {
                let _ = done_tx1.try_send(());
                    ...
                    match Command::new("sh")
                        .args(["-c", &format!("gst-launch-1.0 ximagesrc use-damage=0 ! video/x-raw,width=1366,height=768,framerate=60/1 ! videoconvert ! queue ! x264enc tune=zerolatency speed-preset=superfast bitrate=3000 key-int-max=60 ! video/x-h264, profile=baseline ! rtph264pay pt=96 mtu=1200 ! udpsink host=127.0.0.1 port={}", port)])
                        .kill_on_drop(true)
                        .spawn() {
                            Ok(child) => {
