        ...
        move |connection_state: RTCIceConnectionState| {
            ...
            if connection_state == RTCIceConnectionState::Failed {
                let _ = done_tx1.try_send(());
                    ...
                    let args = &format!("{} ! videoconvert n-threads=4 ! video/x-raw,format=NV12 ! queue ! x264enc threads=4 aud=true b-adapt=false bframes=0 insert-vui=true key-int-max=180 rc-lookahead=0 vbv-buf-capacity=120 sliced-threads=true byte-stream=true pass=cbr speed-preset=veryfast tune=zerolatency qos=true bitrate=3000  ! video/x-h264,profile=high,stream-format=byte-stream ! rtph264pay pt=96 mtu=1200 aggregate-mode=zero-latency config-interval=-1 ! application/x-rtp,media=video,clock-rate=90000,encoding-name=H264,payload=100,rtcp-fb-nack-pli=true,rtcp-fb-ccm-fir=true,rtcp-fb-x-gstreamer-fir-as-repair=true ! udpsink host=127.0.0.1 port={}", {
